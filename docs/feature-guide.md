...
데이터베이스개론 과제 제출용 기능 설명서임. 본 문서는 `DatabaseLanguage_NodeJS_CWNU-Community` 프로젝트의 페이지별 기능, 동작 방식, 관련 API/DB, 캡처 위치, 검증 내용을 정리함. 현재 문서는 v1.2.0 기준 UI/UX 확장 기능과 v2.0.0 계정 구조 리팩터링, 최종 시연용 seed 데이터 구성을 함께 반영함.
...
개발 및 기능 설명서 캡처를 위해 테스트 데이터를 자동 생성하는 seed 스크립트를 제공함. v2.0.0 기준 seed는 loginId 기반 테스트 계정과 함께 메인 대시보드, 인기글 TOP 3, 공지사항, 최근글, 검색 하이라이트, 마이페이지 활동 시각화가 자연스럽게 보이도록 재구성함.
