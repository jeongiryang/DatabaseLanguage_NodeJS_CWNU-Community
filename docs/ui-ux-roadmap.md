# v2.0.0 계정 구조 리팩터링 및 UI/UX 로드맵

## 1. 문서 목적

- `v1.1.0`, `v1.2.0`, `v2.0.0` 후보 작업에서 진행한 CWNU Community UI/UX 및 계정 구조 개선 작업의 완료 상태를 정리함.
- 기능 추가보다 사용성, 정보 탐색, 모바일 접근성, 상태 피드백, 문서 증빙 완성도를 기준으로 정리함.
- v1.2.x까지는 UI/UX와 문서 자산 중심으로 고도화했고, v2.0.0 후보 작업에서는 계정 식별자를 loginId로 전환하는 DB/Auth 구조 변경을 별도 범위로 분리함.
...
| `v2.0.0` 후보 | 구현 완료, tag/Release 대기 | email 기반 계정 식별자를 loginId로 전환하고 마이페이지 비밀번호 변경 기능 추가 |
...
| 항목 | 완료 내용 | 주요 범위 |
|---|---|---|
| loginId 계정 전환 | 회원가입/로그인 식별자를 이메일에서 아이디로 변경하고 `@` 없는 아이디를 허용 | `prisma/schema.prisma`, auth API, `public/login.html`, `public/register.html` |
| 비밀번호 변경 | 마이페이지에서 현재 비밀번호 확인 후 새 비밀번호로 변경하는 API/UI 추가 | `server/controllers/auth.controller.js`, `public/mypage.html`, `public/js/mypage.js` |
| seed 재구성 | 테스트 계정을 `assignment`, `algorithm` 같은 아이디 기반으로 변경 | `scripts/seed-dev.js` |
| 문서 최신화 | README, feature-guide, ai-usage에 loginId 기반 흐름과 비밀번호 변경 기능 반영 | `README.md`, `docs/*.md` |
...
| 후보 | 설명 |
|---|---|
| v2.0.0 스크린샷 갱신 | 아이디 로그인과 비밀번호 변경 화면을 최신 캡처로 교체 |
...
- `v2.0.0` 후보는 기존 UI/UX 레이아웃을 유지하면서 계정 식별자를 loginId로 전환하고 비밀번호 변경 기능을 추가한 구조 개선 단계임.
