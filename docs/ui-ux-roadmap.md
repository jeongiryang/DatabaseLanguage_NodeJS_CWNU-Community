...
- `v1.1.0`, `v1.2.0`, `v2.0.0`에서 진행한 CWNU Community UI/UX 및 계정 구조 개선 작업의 완료 상태를 정리함.
- 기능 추가보다 사용성, 정보 탐색, 모바일 접근성, 상태 피드백, 문서 증빙 완성도를 기준으로 정리함.
- v1.2.x까지는 UI/UX와 문서 자산 중심으로 고도화했고, v2.0.0에서는 계정 식별자를 loginId로 전환하는 DB/Auth 구조 변경을 별도 범위로 분리함.
...
- `v2.0.0`은 기존 UI/UX 레이아웃을 유지하면서 계정 식별자를 loginId로 전환하고 비밀번호 변경 기능을 추가한 구조 개선 단계임.
